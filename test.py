"""
Test script for PointNet2 corner detection with DBSCAN clustering refinement and evaluation.
...
5. Saves refined predictions for visualization
6. Evaluates all test samples using ground truth data
7. Computes metrics: ACO (Average Corner Offset), CP (Corners Precision), 
   CR (Corners Recall), C-F1 (Corners F1-Score)
8. Saves evaluation results to output/results.txt

Usage:
    python test.py --model_path output/corner_detection_model.pth --eps 0.05
    python test.py --skip_eval  # Skip evaluation step
    python test.py --eval_distance_thresh 0.5  # Custom distance threshold for evaluation
"""
...
def evaluate_single_sample(sample_id, refined_corners, gt_vertices, gt_edges, distance_thresh=1.0):
    """
    Evaluate a single sample and compute metrics
    
    Args:
        sample_id: Sample identifier
        refined_corners: Predicted corner coordinates [M, 3] (normalized)
        gt_vertices: Ground truth corner coordinates [N, 3] (normalized)
        gt_edges: Ground truth edges [E, 2]
        distance_thresh: Distance threshold for matching
    
    Returns:
        Dictionary containing metrics for this sample
    """
    from eval.ap_calculator import APCalculator
    from datasets import building3d
    
    # Prepare predicted data (corners only, no edges)
    pd_vertices = refined_corners
    pd_edges = np.empty((0, 2), dtype=np.int32)
    pd_edges_vertices = np.empty((0, 2, 3), dtype=np.float64)
    
    # Prepare ground truth edge vertices
    ...
    distance = ap_calculator.ap_dict['distance']
    
    # Calculate individual metrics
    metrics = {
        'sample_id': sample_id,
        'tp_corners': tp_corners,
        'tp_fp_corners': tp_fp_corners,
        'tp_fn_corners': tp_fn_corners,
        'distance': distance,
        'aco': distance / tp_corners if tp_corners > 0 else 0.0,
        'cp': tp_corners / tp_fp_corners if tp_fp_corners > 0 else 0.0,
        'cr': tp_corners / tp_fn_corners if tp_fn_corners > 0 else 0.0,
    }
    
    # Calculate F1 score
    if metrics['cp'] + metrics['cr'] > 0:
        metrics['c_f1'] = 2 * metrics['cp'] * metrics['cr'] / (metrics['cp'] + metrics['cr'])
    else:
        metrics['c_f1'] = 0.0
...
def evaluate_all_samples(results, test_dataset, dataset_config, 
                         preprocessed_dir='datasets/preprocessed',
                         distance_thresh=1.0):
    """
    Evaluate all test samples and compute average metrics
    ...
    Returns:
        Dictionary containing average metrics across all samples
    """
    from datasets import building3d
    ...
    all_metrics = []
    test_dir = dataset_config.root_dir
    
    for scan_idx, result in results.items():
        try:
            # Load ground truth wireframe
            gt_wireframe_path = os.path.join(test_dir, 'test', 'wireframe', f'{scan_idx}.obj')
            if not os.path.exists(gt_wireframe_path):
                print(f"⚠ Warning: Ground truth wireframe not found for sample {scan_idx}")
                continue
            ...
            preprocessed_path = os.path.join(preprocessed_dir, 'test', f'{scan_idx}.npz')
            if not os.path.exists(preprocessed_path):
                print(f"⚠ Warning: Preprocessed data not found for sample {scan_idx}")
                continue
            ...
            print(f"⚠ Error evaluating sample {scan_idx}: {e}")
            continue
    ...
    avg_metrics = {
        ...
        'aco': np.mean([m['aco'] for m in all_metrics]),
        'cp': np.mean([m['cp'] for m in all_metrics]),
        'cr': np.mean([m['cr'] for m in all_metrics]),
        'c_f1': np.mean([m['c_f1'] for m in all_metrics]),
        'total_tp_corners': sum([m['tp_corners'] for m in all_metrics]),
        'total_tp_fp_corners': sum([m['tp_fp_corners'] for m in all_metrics]),
        'total_tp_fn_corners': sum([m['tp_fn_corners'] for m in all_metrics]),
    ...
    print("="*60)
    print(f"Number of samples evaluated: {avg_metrics['num_samples']}")
    print()
    print(f"Average Corner Offset (ACO): {avg_metrics['aco']:.6f}")
    print(f"Corners Precision (CP):       {avg_metrics['cp']:.6f}")
    print(f"Corners Recall (CR):          {avg_metrics['cr']:.6f}")
    print(f"Corners F1-Score (C-F1):      {avg_metrics['c_f1']:.6f}")
    print()
    print(f"Total statistics:")
    print(f"  True positive corners:  {avg_metrics['total_tp_corners']}")
    print(f"  Predicted corners:      {avg_metrics['total_tp_fp_corners']}")
    print(f"  Ground truth corners:   {avg_metrics['total_tp_fn_corners']}")
    print(f"  Total distance:         {avg_metrics['total_distance']:.4f}")
    print("="*60)
    
    return avg_metrics, all_metrics


def save_evaluation_results(avg_metrics, all_metrics, output_file='output/results.txt'):
    """
    Save evaluation results to a text file
    
    Args:
        avg_metrics: Dictionary containing average metrics
        all_metrics: List of dictionaries containing per-sample metrics
    ...
    with open(output_file, 'w') as f:
        f.write("="*60 + "\n")
        f.write("BUILDING3D CORNER DETECTION - TEST EVALUATION RESULTS\n")
        f.write("="*60 + "\n\n")
        
        # Average metrics
        f.write("AVERAGE METRICS (across all test samples)\n")
        f.write("-"*60 + "\n")
        f.write(f"Number of samples: {avg_metrics['num_samples']}\n\n")
        f.write(f"Average Corner Offset (ACO): {avg_metrics['aco']:.6f}\n")
        f.write(f"Corners Precision (CP):       {avg_metrics['cp']:.6f}\n")
        f.write(f"Corners Recall (CR):          {avg_metrics['cr']:.6f}\n")
        f.write(f"Corners F1-Score (C-F1):      {avg_metrics['c_f1']:.6f}\n\n")
        
        # Total statistics
        f.write("TOTAL STATISTICS\n")
        f.write("-"*60 + "\n")
        f.write(f"True positive corners:  {avg_metrics['total_tp_corners']}\n")
        f.write(f"Predicted corners:      {avg_metrics['total_tp_fp_corners']}\n")
        f.write(f"Ground truth corners:   {avg_metrics['total_tp_fn_corners']}\n")
        f.write(f"Total distance:         {avg_metrics['total_distance']:.4f}\n\n")
        ...
        # Header
        f.write(f"{'Sample ID':<15} {'ACO':<12} {'CP':<12} {'CR':<12} {'C-F1':<12}\n")
        f.write("-"*60 + "\n")
        
        # Sort by sample ID for better readability
        sorted_metrics = sorted(all_metrics, key=lambda x: str(x['sample_id']))
        
        for m in sorted_metrics:
            f.write(f"{str(m['sample_id']):<15} "
                   f"{m['aco']:<12.6f} "
                   f"{m['cp']:<12.6f} "
                   f"{m['cr']:<12.6f} "
                   f"{m['c_f1']:<12.6f}\n")
...
def main():
    ...
    parser.add_argument('--eval_distance_thresh', type=float, default=0.1,
                        help='Distance threshold for evaluation metrics (default: 1.0)')
    parser.add_argument('--skip_eval', action='store_true',
                        help='Skip evaluation after inference (default: False)')
    ...
    # Evaluate all samples and save metrics
    if not args.skip_eval:
        ...
        eval_results = evaluate_all_samples(
            results=results,
            test_dataset=building3D_dataset['test'],
            dataset_config=dataset_config.Building3D,
            preprocessed_dir='datasets/preprocessed',
            distance_thresh=args.eval_distance_thresh
        )
        
        if eval_results is not None:
            avg_metrics, all_metrics = eval_results
