...
# python train.py --resume output/checkpoint_epoch_290.pth --wandb_id d0uens16


def train_model(train_loader, dataset_config, input_channels=None, resume_from_checkpoint=None):
    """
...
def train_with_preprocessed_data(resume_from_checkpoint=None, wandb_run_id=None):
    """
    Function that loads preprocessed data and passes it to train_model for PointNet2 training
    
    Args:
        resume_from_checkpoint: Path to checkpoint file to resume from (e.g., 'output/checkpoint_epoch_100.pth')
        wandb_run_id: WandB run ID to resume (if None, will create new run or auto-resume)
    """
    ...
    project="Building3D",
    # Resume settings
    id=wandb_run_id,  # Use specific run ID if provided
    resume="allow",    # Allow resuming if id matches existing run, otherwise create new
    # Track hyperparameters and run metadata.
    config={
        "learning_rate": 0.001,
        "architecture": "PointNet2",
        "dataset": "Buildin3D-Entry-Level",
        "epochs": 300,
        },
    )   
    ...
    model = train_model(train_loader, dataset_config, calculated_input_dim, resume_from_checkpoint)
...
def main():
    ...
    To resume training from a checkpoint:
        python train.py --resume output/checkpoint_epoch_100.pth --wandb_id YOUR_RUN_ID
    
    Or set these in the code directly.
    """
    import argparse
